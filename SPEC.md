...
- has json config file for customization
  ...
  - autoAcceptCommitMessage: boolean to determine if the generated commit message should be automatically accepted or if the user should be prompted to accept it.
...
The tool asks the user for commit message, if no mesage given and user presses enter then: The tool generates a commit message from the staged diff using a model (Pressing tab will change model), if the commit message is generated by the llm then ask the user for a confirmation, press y/n/enter to confirm. or can press `r` to retry generating the message if not satisfied with the generated message. if y/enter then commits, if no then exit the app
...
- Use ai sdk "ai" for doing things with llm
...
- Use "https://www.npmjs.com/package/noarg" for the argument aprser..
- Use "https://www.npmjs.com/package/@inquirer/prompts" for the interactive prompts
- Use "chalk" for coloring the terminal output
- Use 'zod' for validating the config file
- All the dependencies should be added as dev dependencies since this is a cli tool and we will compile it to js before publishing, so the users will not need to install the dependencies themselves.
